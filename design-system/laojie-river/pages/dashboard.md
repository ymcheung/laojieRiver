...
- **Top Command Row:** On vault screens, the global top-right command row is ordered Search items, Lock, Settings. Keep Search and Lock immediately to the left of Settings instead of inside the split-pane list header. Use a shared 36px control height, 10px horizontal gaps, and padding-based container spacing rather than bottom margins. The row owns the mobile inline space from the viewport edges, right-aligns the controls, and lets Search shrink before controls overlap; preserve visible focus rings on Search, Lock, and Settings.
- **Resizable Divider:** On desktop, the vertical Zinc bar between the panes is an interactive splitter. Support pointer drag and keyboard arrow resizing, and clamp pane widths so the left pane remains at least 320px and the right pane remains at least 512px on vault screens.
- **Vault Navigation Placement:** From the `lg` desktop width (1024px) upward, the All Items/Favorites/Logins/Secure Notes/Cards/Identities/2FA navigation sits outside the split-pane workspace as a separate left rail. Reserve an 8rem rail plus a 1rem gap, and set the workspace start to `max(wireframe-left, nav-left + nav-width + nav-gap)` so the rail never overlaps the list/detail workspace.

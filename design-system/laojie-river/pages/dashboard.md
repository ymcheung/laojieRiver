...
- **Top Command Row:** On vault screens, the global top-right command row is ordered Search items, Lock, Settings. Keep Search and Lock immediately to the left of Settings instead of inside the split-pane list header. The row owns the mobile inline space from the viewport edges, right-aligns the controls, and lets Search shrink before controls overlap; preserve visible focus rings on Search, Lock, and Settings.
